...
func deleteObject(tx *txn, bid int64, name string, version string) (row deletedRow, found bool, _ error) {
	...
	err := tx.QueryRow(`
		...
		RETURNING sia_object_id, filename, size, content_md5, updated_at, is_delete_marker, is_latest, created_at_gen
	`, bid, name, version).Scan(&id, &row.filename, &row.size, (*sqlMD5)(&row.contentMD5), (*sqlTime)(&row.updatedAt), &row.isDeleteMarker, &wasLatest, &row.createdAtGen)
	if errors.Is(err, sql.ErrNoRows) {
