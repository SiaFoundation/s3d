...
func TestSelectSnapshot(t *testing.T) {
	...
	snapshots := []sia.RemoteSnapshot{remote(version), remote(version - 1)}
	...
	// latest picks the newest snapshot, which enumeration returns first
	...
	if _, err := selectSnapshot(snapshots, snapshots[1].ObjectID.String()); err != nil {
		t.Fatal(err)
	}
